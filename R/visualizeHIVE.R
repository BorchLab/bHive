...
visualizeHIVE <- function(result, 
  ...
  if (!is.null(X) && plot_type == "scatter") {
    ...
    feat_cols <- setdiff(seq_len(ncol(proto_all)), which(names(proto_all) %in% c("Layer", "Group")))
    ...
    if (task == "classification") {
      ...
      grouping <- df_groups$Group
    }
    
    # Build the data frame for the feature.
    if (is.null(feature)) {
    ...
    df_feature <- data.frame(Feature = rep(feature_values, times = length(selectedLayers)),
                             ...
                             Group = rep(grouping, times = length(selectedLayers)))
    
    # Prepare prototype data for the selected feature.
    if (is.numeric(feature)) {
      proto_feature <- proto_all[, feature]
    } else {
      proto_feature <- proto_all[, feature_name]
    }
