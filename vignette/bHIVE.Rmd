...
2. Tune hyperparameters using `tune_bHIVE`.
...
ggplot(iris, aes(x = Sepal.Length, 
                 ...
                 shape = Species)) +
    geom_point(size = 3) +
    labs(title = "Classification Results with bHIVE", x = "Sepal Length", y = "Sepal Width") +
    theme_minimal()
```
...
# Perform regression
set.seed(123)
res <- bHIVE(X = X, y = y, task = "regression", nAntibodies = 50, beta = 5, epsilon = 0.01, maxIter = 20, k = 3)
...
ggplot(Boston, aes(x = y, y = Predicted)) +
    ...
    labs(title = "Regression Results with bHIVE", x = "Actual Values (medv)", y = "Predicted Values") +
    theme_minimal()
...
# Perform tuning
set.seed(123)
tuning_results <- tune_bHIVE(X = X, task = "clustering", grid = grid, metric = "silhouette")
...
ggplot(tuning_results$results, aes(x = nAntibodies, y = metric_value, color = factor(beta))) +
    geom_line() +
    geom_point(size = 3) +
    labs(title = "Hyperparameter Tuning Results", x = "Number of Antibodies", y = "Silhouette Score", color = "Beta") +
    theme_minimal()
```
...
ggplot(model) +
    labs(title = "Caret Tuning Results for bHIVE", x = "Hyperparameter Combination", y = "Performance Metric")
```
